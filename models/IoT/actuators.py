...
class Actuator(db.Model):
    ...
    def save_actuator(name, brand, model, topic, unit, is_active):
        ...
        actuator = Actuator(devices_id = device.id, topic = topic, unit = unit)
    ...
    def get_actuators():
        actuators = Actuator.query.join(Device, Device.id == Actuator.devices_id)\
            .add_columns(Device.id, Device.name, Device.brand, Device.model, Device.is_active, Actuator.topic, Actuator.unit)\
            .all()
    ...
    def get_single_actuator(id):
        ...
        if actuator is not None:
            actuator = Actuator.query.filter(Actuator.devices_id == id)\
                    .join(Device).add_columns(Device.id, Device.name, Device.brand, Device.model, Device.is_active, Actuator.topic, Actuator.unit)\
                    .first()
